...
static t_router_dto			*create_router_dto_bundle(cherokee_t *instance)
{
	...
	bundle = malloc(sizeof(t_router_dto));
	if (!bundle) {
		return (NULL);
	}
	bundle->client = -1;
...
int							cherokee_run(cherokee_t *cherokee)
{
	...
	while (cherokee->running)
	{
		...
		count = select(max + 1, &read_fs, NULL, NULL, &timeval);
		if (count > 0)
		{
			node = cherokee->clients->head;
			while (node)
		...
		if ((end - start) > 15)
		  {
			cherokee->running = 0;
			thpool_wait(cherokee->pool);
		    int client = socket(AF_INET, SOCK_STREAM, 0);
		    struct sockaddr_in addr_in;
		    addr_in.sin_family = AF_INET;
		    addr_in.sin_addr.s_addr = INADDR_ANY;
		    addr_in.sin_port = htons(PORT);
		    connect(client, (struct sockaddr *)&addr_in, sizeof(addr_in));
		    close(client);
		  }
#endif
	}
